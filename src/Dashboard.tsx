...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  // ✅ All state definitions must live inside the component
  const [enrollments, setEnrollments] = useState<Enrollment[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');

  // ✅ The data fetch function lives here and correctly uses the state hooks
  const fetchEnrollments = async () => {
    setLoading(true);
    try {
      // Dynamically checks if running locally or live on Vercel production
  ...
  // Filter logic runs on every render safely
  ...
  // Calculates total accumulated revenue dynamically
  const totalRevenue = enrollments.reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-start justify-between">
            <div>
              <p className="text-slate-500 text-sm font-medium mb-1">Total Enrollments</p>
              <h2 className="text-4xl font-bold text-slate-900">{enrollments.length}</h2>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-start justify-between">
            <div>
              ...
              <h2 className="text-4xl font-bold text-slate-900">{enrollments.filter(e => e.status?.toLowerCase() === 'paid').length || enrollments.length}</h2>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-start justify-between">
            <div>
              ...
              <h2 className="text-4xl font-bold text-slate-900">₹{totalRevenue.toLocaleString()}</h2>
            </div>
